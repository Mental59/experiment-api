...
def signup(user: schemas.UserSignup, db: Session = Depends(get_db)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail=create_exception_details(message="Пользователь с таким логином уже существует"))
...
def signin(signin_data: schemas.UserSignin, db: Session = Depends(get_db)) -> Token:
    ...
    if not user:
        raise HTTPException(
            ...
            detail=create_exception_details(message="Неверный логин или пароль"),
